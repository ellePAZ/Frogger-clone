...
Car::Car(const sf::Vector2f p_startPos, int p_size) : Vehicle(p_startPos, p_size)
{
	...
	// Vertex graphic design
	{
		...
		m_shape[0].position = sf::Vector2f(midPoint.x - m_size * 0.4f, midPoint.y - m_size * 0.22f);
		m_shape[1].position = sf::Vector2f(midPoint.x - m_size * 0.4f, midPoint.y + m_size * 0.22f);
		m_shape[2].position = sf::Vector2f(midPoint.x + m_size * 0.4f, midPoint.y - m_size * 0.22f);
		m_shape[3].position = sf::Vector2f(midPoint.x + m_size * 0.4f, midPoint.y + m_size * 0.22f);
		//m_shape[4].position = sf::Vector2f(midPoint.x - m_size * 0.3f, midPoint.y - m_size * 0.25f);
		m_shape[4].position = sf::Vector2f(midPoint.x + m_size * 0.6f, midPoint.y);
...
Vehicle* Car::clone()
{
	return new Car(this->getPosition(), m_size);
}